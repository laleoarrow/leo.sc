---
title: "Functional Inference Pipeline (FIP) — SKO Tutorial"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Functional Inference Pipeline (FIP) — SKO Tutorial}
...
3. **Select g^high**: Select the top cells (top `pct_threshold`% or `abs_threshold` cells) as the **high-expression group**, preserving cell-type composition.
...
5. **DEG**: Apply `Seurat::FindMarkers()` (Wilcoxon rank-sum; *g^low* as `ident.1`) to identify differentially expressed programs between groups.
