...
The demo above uses `pbmc_small` (80 cells) where FIP cannot produce
meaningful enrichment. Below we walk through a **real-world FIP result** from
...
reflect variation in LRRK2 expression *within each lineage* rather than
confounding by cell-type abundance. Let's inspect:
...
knitr::kable(
  ...
  digits = 3,
  caption = "Top 10 up-regulated DEGs (LRRK2-low vs LRRK2-high)"
)
```
...
> **Key biological insight.** Although LRRK2 is primarily recognised as a
> Parkinson's disease susceptibility gene, FIP reveals its transcriptional
> impact in **myeloid immune cells** — highlighting antigen binding, oxidative
> phosphorylation, and leukocyte-mediated cytotoxicity as the top enriched
> programmes. These computationally inferred pathways are consistent with
> experimental evidence from Gardet *et al.*, who demonstrated that LRRK2
> governs inflammatory cytokine production via NF-κB phosphorylation and
> **antigen presentation** in bone marrow-derived dendritic cells
> ([*Int J Mol Sci* 2020;21:6097](https://doi.org/10.3390/ijms21176097)).
> This concordance between *in silico* inference and wet-lab perturbation
> underscores FIP's utility for generating testable hypotheses from
