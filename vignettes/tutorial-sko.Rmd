...
Below we show a **real-world FIP result** from a Vogt–Koyanagi–Harada (VKH) disease study, 
applying `silico_ko()` to **LRRK2** in a scRNA-seq cohort of 34 VKH patients and 14 healthy controls.
...
```{r lrrk2-gsea-plot, echo=FALSE, out.width="100%", fig.cap="GSEA enrichment plot for LRRK2-high vs LRRK2-low (GO)."}
# Dynamic download to keep the R package lightweight
data_url <- "https://github.com/laleoarrow/leo.sc/releases/download/data-v2/1.LRRK2_ko_res.qs"
# Use a persistent cache directory to avoid re-downloading
cache_dir <- tools::R_user_dir("leo.sc", which = "cache")
if (!dir.exists(cache_dir)) dir.create(cache_dir, recursive = TRUE)
dest_file <- file.path(cache_dir, "1.LRRK2_ko_res.qs")

if (!file.exists(dest_file)) {
  message("Downloading real-world LRRK2 data (4.9 MB) from GitHub...")
  utils::download.file(data_url, dest_file, mode = "wb", quiet = TRUE)
}

# Load precomputed FIP result
res <- qs::qread(dest_file)
...
> oxidative phosphorylation, and cytotoxicity. Notably, these computationally
> inferred programmes align with experimental evidence: Gardet *et al.*
> demonstrated that LRRK2 controls inflammatory cytokine production through
> NF-κB phosphorylation and **antigen presentation** in bone marrow-derived
> dendritic cells
> ([*Int J Mol Sci*, 2020](https://doi.org/10.3390/ijms21176097)),
> validating FIP's prediction of antigen binding as the top enriched
> pathway. This concordance between *in silico* inference and wet-lab
> perturbation underscores FIP's utility for hypothesis generation from
> observational single-cell data.
