...
#' @param layer       Expression layer to inspect: \code{"counts"} for presence; \code{"data"} for averaged expression.
...
#' }
#' @export
check_gene_stats_in_multi_batch <- function(srt, batch_col = "Batch", layer = "counts") {
  gs   <- SplitObject(srt, split.by = batch_col) |>
    lapply(\(x) rownames(x)[Matrix::rowSums(GetAssayData(x, layer = layer) > 0) > 0])
...
#' @rdname check_gene_stats_in_multi_batch
#' @export
check_gene_avg_in_multi_batch <- function(srt, genes, batch_col = "Batch", layer = "counts") {
  m <- AverageExpression(srt, features = genes, group.by = batch_col, layer = layer, verbose = FALSE)[[DefaultAssay(srt)]]
